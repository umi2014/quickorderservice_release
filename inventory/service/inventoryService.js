...
const inventoryService = {
    ...
    async overview(shopId, langType) {
      ...
      `;
        const [products] = await pool.query(sql, [langType || 1, shopId, shopId]);
    ...
    async getProduct(shopId, productId, langType) {
      ...
      `;
        const [rows] = await pool.query(sql, [langType || '0', shopId, productId]);
    ...
    async getAllProducts(shopId, langType) {
      ...
      `;
        const [rows] = await pool.query(sql, [langType || '1', shopId]);
    ...
    async createProduct(shopId, data) {
        return await pool.transaction(async () => {
          ...
          `;
            ...
            VALUES (?, ?, ?)
          `;
            await pool.query(insertLangSql, [productId, langType, productName]);
